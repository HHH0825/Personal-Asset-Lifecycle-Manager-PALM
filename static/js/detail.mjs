import { $, methodNames, iconTypes, iconMarkup, decorativeIcon, itemVisual, escapeHtml, yuan, badge } from './common.mjs';
...
function renderDetail(item, currentUser) {
  ...
  $('#detail-content').innerHTML = `
    <div class="detail-hero">
      <div class="detail-top">${itemVisual(item, 'item-icon')}<div class="detail-main"><h2>${item.photo_url ? `<span class="inline-type-icon">${iconMarkup(item.icon_type)}</span>` : ''}${escapeHtml(item.name)} ${badge(item.status)}</h2><p>${escapeHtml(item.category)} · ${iconTypes[item.icon_type] || iconTypes.other} · 购买于 ${item.purchase_date}</p><p class="detail-warranty">保修到期：${item.warranty_expires_on || '未填写'}</p></div><div class="detail-actions"><button class="small-btn" data-action="edit-item">编辑物品</button><button class="small-btn danger" data-action="delete-item">删除物品</button></div></div>
