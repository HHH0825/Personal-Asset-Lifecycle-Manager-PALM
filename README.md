...
1. 未登录打开介绍页，拖动键盘示例至第 150／300／365 天，观察费用与目标进度。
2. 从介绍页注册账号，确认进入空档案；向空账号导入演示数据后刷新。
3. 查看充电宝的百日纪念章和当天达成目标、键盘明天达成目标、笔记本已达成目标、自行车处置后冻结的目标。
4. 设置或修改目标，回到物品清单核对提示；进入数据概览、待复盘，继续演示维修和处置操作。
5. 退出后返回介绍页，注册第二个账号，确认档案与统计为空；重新登录原账号确认数据仍在。
6. 在手机宽度检查介绍页、档案、详情与表单；重启服务后验证目标和记录保留。
