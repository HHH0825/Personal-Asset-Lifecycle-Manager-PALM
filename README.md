...
页面使用暖白、墨绿和淡彩标签，桌面物品卡片双列、窄屏单列；录入、详情、统计及认证页面使用同一套风格。插画和图标是本地 SVG，字体采用系统字体；动画遵循系统“减少动态效果”设置，无需外部字体、图片或图标服务。

以下为 2026-09-22 使用演示数据截取的页面效果；实际物品、金额和持有天数会随账号数据及日期变化。
